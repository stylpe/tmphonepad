...
internal sealed class Controller : IDisposable
{
    ...
    {
        if (!result.EndOfMessage) throw new InvalidDataException("Received a partial message... Surely this will never happen in practice?");
        if (result.Count != 2) throw new InvalidDataException($"Message length was invalid: {result.Count} (should always be 2)");
    }
